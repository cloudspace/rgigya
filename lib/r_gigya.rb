...
#
# Example call
# Gigya.socialize_setStatus(:uid => @user.id,:status => 'hello')
#
...
class RGigya
  ...
  # When in use please namespace them appropriately Gigya::ResponseError for readability
  #
  exceptions = %w[ UIDParamIsNil SiteUIDParamIsNil ResponseError BadParamsOrMethodName ErrorCodeReturned  ]  
  exceptions.each { |e| const_set(e, Class.new(StandardError)) }
  Gigya::JSONParseError = Class.new(JSON::ParserError)
  ...
  class << self
    ...
    def build_url(method, options = {})
      ...
      if options.has_key?(:uid) && options[:uid].nil?
        raise Gigya::UIDParamIsNil, ""
      end
      
      if options.has_key?(:siteUID) && options[:siteUID].nil?
        raise Gigya::SiteUIDParamIsNil, ""
      end
    ...
    def parse_results(method, options = {})
      ...
      # rescue Gigya::ResponseError, Gigya::SiteUIDParamIsNil, Gigya::UIDParamIsNil => e 
      # rescue Gigya::ResponseError => e 
      rescue SocketError,Timeout::Error => e 
        raise Gigya::ResponseError, e.message
      end
      ...
      begin
        ...
        raise Gigya::JSONParseError, e.message
      end
      doc
    end
    ...
    def check_for_errors(results)
      case results['errorCode'].to_s
        ...
        when '400002'
          raise Gigya::BadParamsOrMethodNames
        else 
          log("Gigya returned Error code #{results['errorCode']}.\n\nError Message: #{results['errorMessage']}\n\nError Details: #{results['errorDetails']}")
          raise Gigya::ErrorCodeReturned, "returned Error code #{results['errorCode']}: #{results['errorMessage']}"
      end
    end
